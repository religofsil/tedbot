...
from getdata import _read_data, processRequest, Video
...
def swearwords(q):
    f = open(DIR + config.WORDLIST, 'r', encoding='utf-8')
    for line in f:
...
def get_key_words(text):
    ...
    result = processRequest('post', config.KEYWORDS_URL, json, data, headers, params)
...
def rand2(message):
    ...
    if video is None:
        text = random.choice(phrases.no_match)
    else:
        text = video.URL
...
def getvideo(message):
    if regRus.search(message.text) is not None:
        ...
        if text is None:
            bot.send_message(message.chat.id, random.choice(phrases.no_match))
        else:
